...
class Tetris:
    ...
    BOARD_WIDTH = 10
    BOARD_HEIGHT = 20
    RIGHT_PANEL_WIDTH = 6  # Width of the right panel for displaying scores and info
    ...
    COLORS = {
        0: (0, 0, 0),   # Colour for empty squares
        1: (0, 0, 255),     # Colour for blocks
        2: (255, 0, 0),     # Colour for current piece
    }
    ...
    def _render_right_panel(self, img):
        '''Render the right panel with scores, pieces played, and lines cleared'''
        panel_width = Tetris.RIGHT_PANEL_WIDTH * 25
        panel_height = Tetris.BOARD_HEIGHT * 25
        panel = np.ones((panel_height, panel_width, 3), dtype=np.uint8) * 255  # White background

        # Render scores
        cv2.putText(panel, f"Score: {self.score}", (5, 50), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
        cv2.putText(panel, f"Lines: {self.lines_cleared}", (5, 100), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)
        cv2.putText(panel, f"Pieces: {self.pieces_played}", (5, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0), 1)

        # Concatenate right panel with the main image
        img = np.concatenate((img, panel), axis=1)
        return img
    ...
    def render(self):
        ...
        img = np.array(img)
        img = self._render_right_panel(img)  # Render right panel
...
# Sample usage of the Tetris class to play the game randomly
